...
result = pf.feols("mrate ~ legal + C(state) + C(year)", data=allcause, vcov={"CRV1": "state"})
...
for dtype_val, label in dtype_labels.items():
    ...
    r1 = pf.feols("mrate ~ legal + C(state) + C(year)", data=s, vcov={"CRV1": "state"})
    ...
    r2 = pf.feols("mrate ~ legal + C(state) + C(year) + C(state):year", data=s, vcov={"CRV1": "state"})

    # Spec 3: Population-weighted (WLS)
    # Weight by state population so larger states count more (more reliable death rates)
    r3 = pf.feols("mrate ~ legal + C(state) + C(year)", data=s, weights="pop", vcov={"CRV1": "state"})
...
for dtype_val, label in [("all", "All causes"), ("MVA", "Motor vehicle")]:
    ...
    r = pf.feols("mrate ~ legal + beertax + C(state) + C(year)", data=s, vcov={"CRV1": "state"})
...
result = pf.feols("mrate ~ legal + C(state) + C(year)", data=allcause, weights="pop", vcov={"CRV1": "state"})
...
result = pf.feols("mrate ~ legal + C(state) + C(year)", data=suicide, vcov={"CRV1": "state"})
...
> r = pf.feols("mrate ~ legal + C(state) + C(year)", data=s).fit(
...
>
> # --- Unweighted OLS ---
> r_uw = pf.feols("mrate ~ legal + C(state) + C(year)", data=allcause).fit(
> cov_type="cluster", cov_kwds={"groups": allcause["state"]})  # cluster SEs by state
>
> # --- Population-Weighted WLS ---
> r_wt = pf.feols("mrate ~ legal + C(state) + C(year)", data=allcause,
...
>
> # --- Baseline DD ---
> r_base = pf.feols("mrate ~ legal + C(state) + C(year)", data=mva).fit(
...
> r_trend = pf.feols("mrate ~ legal + C(state) + C(year) + C(state):year_num", data=mva).fit(
> cov_type="cluster", cov_kwds={"groups": mva["state"]})
>
> # --- Compare ---
> pd.DataFrame({
