...
for dtype_val, label in [("all", "All causes"), ("suicide", "Suicide")]:
    ...
    r = pf.feols("mrate ~ legal + C(state) + C(year)", data=s, vcov={"CRV1": "state"})
...
# --- Unweighted OLS ---
r_uw = pf.feols("mrate ~ legal + C(state) + C(year)", data=allcause, vcov={"CRV1": "state"})
...
# --- Baseline DD ---
r_base = pf.feols("mrate ~ legal + C(state) + C(year)", data=mva, vcov={"CRV1": "state"})

# --- DD with State-Specific Linear Time Trends ---
mva["year_num"] = mva["year"] - mva["year"].min()  # center year for numerical stability
r_trend = pf.feols("mrate ~ legal + C(state) + C(year) + C(state):year_num", data=mva, vcov={"CRV1": "state"})
