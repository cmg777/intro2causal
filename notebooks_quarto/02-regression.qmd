...
# Short regression (biased) vs long regression (correct)
short2 = smf.ols("earnings ~ private", data=students2).fit()
long2 = smf.ols("earnings ~ private + ability", data=students2).fit()
...
# Stronger ability-private link (divide by 2 instead of 5)
np.random.seed(42)
ability3 = np.random.normal(50, 10, n)
prob3 = 1 / (1 + np.exp(-(ability3 - 50) / 2))  # stronger link
...
short3 = smf.ols("earnings ~ private", data=students3).fit()
long3 = smf.ols("earnings ~ private + ability", data=students3).fit()
aux3 = smf.ols("ability ~ private", data=students3).fit()

ovb3 = round(short3.params["private"] - long3.params["private"])
formula3 = round(aux3.params["private"] * long3.params["ability"])
...
# Add family_income as second confounder
np.random.seed(42)
...
# Three regressions: short, medium (only ability), long (both controls)
r_short = smf.ols("earnings ~ private", data=students4).fit()
r_medium = smf.ols("earnings ~ private + ability", data=students4).fit()
r_long = smf.ols("earnings ~ private + ability + family_income", data=students4).fit()
