...
# ----------------------------------------------------------------
iv = IV2SLS.from_formula("lnw ~ 1 + [s ~ q4]", data=qob).fit(cov_type="robust")
print(f"3. IV (QOB):    {iv.params['s']:.4f}")
...
rd = smf.wls("avgearnings ~ pass_exam * minscore",
             data=bandwidth_data,
             weights=bandwidth_data["person_years"]).fit(cov_type="HC1")
print(f"4. RD:          ${rd.params['pass_exam']:.0f} (Credential effect)")
...
dd_s = smf.ols("avg_schooling ~ treat_post + C(state) + C(year)",
               data=did).fit(cov_type="cluster", cov_kwds={"groups": did["state"]})

# Reduced form: Effect of policy on earnings
dd_e = smf.ols("avg_earnings ~ treat_post + C(state) + C(year)",
               data=did).fit(cov_type="cluster", cov_kwds={"groups": did["state"]})

wald_did = dd_e.params['treat_post'] / dd_s.params['treat_post']
