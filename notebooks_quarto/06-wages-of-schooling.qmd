...
fs_cl = pf.feols("indEduc ~ cl7 + cl8 + cl9 + C(sob) + C(yob) + C(year)", data=cl, weights="weight", vcov={"CRV1": "sob"})
...
earn_l = pf.feols("avgearnings ~ pass_exam + left_1 + left_2 + left_3 + left_4", data=left[left["minscore"] >= -30], weights="person_years")
...
dd_s = pf.feols("avg_schooling ~ treat_post + C(state) + C(year)", data=did, vcov={"CRV1": "state"})

# Reduced form: Effect of policy on earnings
dd_e = pf.feols("avg_earnings ~ treat_post + C(state) + C(year)", data=did, vcov={"CRV1": "state"})
...
iv_multi = pf.feols("lnw ~ 1 + {yob_str} | s ~ q2 + q3 + q4", data=iv_data, vcov="hetero")
...
placebo_dd = pf.feols("avg_earnings ~ placebo_treat_post + C(state) + C(year)", data=pre, vcov={"CRV1": "state"})
