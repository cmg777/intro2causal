...
The table walks through the IV recipe step by step. The reduced form shows that Q4 births earn slightly more (about 0.7% higher log earnings). The first stage shows they also get about 0.09 more years of schooling. Dividing the reduced form by the first stage gives the Wald estimate of about **7% per year of schooling** --- which the 2SLS verification confirms. Let's visualize these patterns across birth cohorts.
...
The sawtooth pattern shows that Q4 births (filled circles) consistently have slightly more education and slightly higher earnings than Q1 births (open circles). The first-stage F-statistic is about 49, well above the weak-instrument threshold of 10. The IV estimate of **about 7%** is close to the OLS estimate, suggesting that ability bias may be modest.
:::
...
The OLS estimate with fixed effects gives about **7%**, while the IV estimate is larger at about **13%**. The IV estimate is less precise than the QOB results, in part because the first-stage F-statistic is below 10 --- a sign of **weak instruments** that can inflate IV estimates. This is a common challenge with compulsory schooling law instruments: the laws affected only a small fraction of the population, so the first stage is modest. Despite the imprecision, the results are broadly consistent with the other IV strategies in pointing to a causal return that is at least as large as the OLS estimate. The compliers here are students whose schooling was changed by compulsory attendance requirements, typically those at the dropout margin.
:::
...
| Quarter-of-birth IV | ~0.07 | LATE for compliers only | --- |
...
8. **Child labor law IV (~7--13%)** uses a different set of instruments, though weak instruments make the IV estimate imprecise.

9. **Sheepskin RD (~0%)** shows the diploma itself has little earnings value --- learning matters more.

10. **DD exploiting policy changes (~6%)** provides yet another perspective using before/after comparisons.
